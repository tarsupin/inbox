...
/****** Get a list of posts within the thread specified ******/
	...
	,	$numRows = 20	// <int> The number of posts to show.
	)					// RETURNS <int:[str:mixed]> an array of posts.
	
	// $posts = AppThread::getPosts($threadID, [$page], [$numRows]);
	{
		return Database::selectMultiple("SELECT p.*, s.signature, u.handle, u.display_name, u.date_joined FROM posts p INNER JOIN settings s ON p.uni_id=s.uni_id INNER JOIN users u ON p.uni_id=u.uni_id WHERE p.thread_id=? ORDER BY p.id ASC LIMIT " . (($page - 1) * $numRows) . ', ' . ($numRows + 0), array($threadID));
	}
...
/****** Get the ID of the folder that the thread belongs to ******/
	...
	(
		$uniID			// <int> The UniID that we're identifying the folder for.
	,	$threadID		// <int> The ID of the thread.
	)					// RETURNS <str:mixed> an array of the folder data.
...
/****** Verify if a user is allowed access to a thread ******/
	...
	(
		$uniID			// <int> The UniID to verify access.
	,	$threadID		// <int> The ID of the thread.
	)					// RETURNS <bool> TRUE if the user can access the thread, FALSE on failure.
...
/****** Get a list of users allowed in this thread ******/
	...
	(
		$threadID		// <int> The ID of the thread.
	)					// RETURNS <int:[str:mixed]> a list of user data.
...
/****** Get a UniID List for users in this thread ******/
	...
	(
		$threadID		// <int> The ID of the thread.
	)					// RETURNS <int:int> a list of user data.
...
/****** Create a new thread ******/
	...
	,	$uniIDList		// <int:int> An array of UniID's that are being added to the thread.
...
/****** Edit an existing Thread ******/
	...
	(
		$folderID		// <int> The folder ID that contains the thread you're editing.
...
/****** Mark a thread as read ******/
	...
	(
		$uniID			// <int> The UniID who is marking the thread read.
	,	$threadID		// <int> The ID of the thread.
	)					// RETURNS <bool> TRUE on success, or FALSE on failure.
...
/****** Mark a thread as unread ******/
	...
	(
		$uniID			// <int> The UniID who is marking the thread unread.
	,	$threadID		// <int> The ID of the thread.
	)					// RETURNS <bool> TRUE on success, or FALSE on failure.
